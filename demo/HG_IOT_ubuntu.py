...
class HGTouch_smartconfig():
    ...
    def Gui_settings(self):
        self.root.title('HG IOT')
        self.root.geometry('400x300')
        self.root.maxsize(410,410)
        self.root.minsize(410,410)

        header = Label(self.root, bg = "orange", width = 300, height = 2)
        header.place(x = 0, y = 0)

        h1 = Label(self.root, text = "HG 物联网平台", bg = "orange", fg = "black", font = ('verdana', 13, 'bold'))
        h1.place(x = 135, y = 5)
    ...
    def Login(self):
            ...
            if len(ssid) != 0 or len(password) != 0 or len(retry) != 0 or len(ip) != 0:
                self.wifi_settings(ssid, password, ip)
